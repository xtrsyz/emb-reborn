...
$content->post_editor_type            = $new['type']           = $params->POST->type ?: 'post';
...
$content->post_editor_date_categories = $new['categories']     = $params->POST->categories;
$content->post_editor_date_tags       = $new['tags']           = $params->POST->tags;
...
foreach($content->get() as $key => $value)
	if(strpos($key, 'post_editor_') === 0)
		unset($content->{$key});
$location = $dashboard->URI('posts', 'edit', $post_id);
